...
- **Singular-value spectra** — live publication-style charts built from the raw `sv_01…` columns of `hankel_sweep.csv` (fetched lazily via `/api/spectra`, absent on runs that predate those columns). Three views, all following the scrubber episode: a linear + normalized log-scree pair for the selected episode, start vs. selected checkpoint (normalized σᵢ/σ₁, with a log-scree inset that survives SVG/PNG export), and the spectrum evolution over training (one curve per analysis tick, light → dark). Normalized views include a dashed Marchenko–Pastur reference — the expected spectrum of an i.i.d. Gaussian matrix of the same shape, i.e. what *full rank* looks like at that size.
