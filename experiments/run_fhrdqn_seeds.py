...
directory as cwd (config paths and runs/ are resolved against cwd):

    cd experiments/dqn_mountaincar
    python ../run_fhrdqn_seeds.py                 # all arms x config seeds, 6-wide
    python ../run_fhrdqn_seeds.py --max-workers 3 # gentler fan-out
    python ../run_fhrdqn_seeds.py --force         # rerun pairs already in the manifest

Child mode (one training in this process; used internally by the launcher):

    python ../run_fhrdqn_seeds.py --arm fhr --seed 21

Completed runs are recorded in fhrdqn_runs_manifest.json (run dirs relative to
the experiment dir), so a relaunch only runs missing/failed pairs. The
...
stdout/stderr goes to logs/fhrdqn_<arm>_seed<N>.log. Children are pinned to one
...
SRC = EXPERIMENTS_DIR.parent / "src"
CONFIG = "config_fhrdqn.yaml"
MANIFEST = "fhrdqn_runs_manifest.json"
ARMS = ("baseline", "fhr")
...
def run_one(arm, seed, episodes=None):
    ...
    logger = make_run_logger(cfg, config_path=CONFIG)
